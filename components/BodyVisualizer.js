...
export function Controls() {
    ...
    const a = (
        <orbitControls ref={ref} args={[camera, gl.domElement]} enableDamping dampingFactor={0.1} rotateSpeed={0.5}/>);
    return a
}
...
const ImagesVisualizer = observer(({images, preview_state}) => {
    return images.pictures.filter(applyFilter).map(x => <ImageVisualizer image={x} preview_state={preview_state}
                                                                         key={x.date}/>)
});
